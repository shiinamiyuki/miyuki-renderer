...
std::shared_ptr<BSDF> BSDFFactory::operator()(const MaterialInfo &materialInfo) {
    ...
    if (kd.maxReflectance > 0) {
        if (materialInfo.roughness <= 1e-4f)
            ...
            bsdf = std::make_shared<OrenNayarBSDF>(materialInfo.roughness, kd);
    }
    if (ks.maxReflectance > 0) {
        ...
        if (alpha <= 1e-4f) {
            bsdf2 = std::make_shared<SpecularBSDF>(fresnel, ks);
        } else {
            Float alphaX, alphaY;
        ...
        CHECK(bsdf2);
        if (bsdf) {
            bsdf = std::make_shared<MixedBSDF>(bsdf, bsdf2, 1.0f);
        } else {
            bsdf = bsdf2;
        }
    }
