...
namespace Miyuki {
	namespace Core {
		Float AreaLight::power() const {
			return primitive->material() ? primitive->material()->emission->average() : {};
		}
		Spectrum AreaLight::L(const Ray& ray) const {
			Intersection isct;
			primitive->intersect(ray, &isct);
			return primitive->material() ? primitive->material()->emission->eval(ShadingPoint(isct.uv)) : {};
		}
		...
		void AreaLight::sampleLi(const Intersection& isct, LightSamplingRecord& record, VisibilityTester* tester) const {
			...
			record.wi = w;
			record.Le = primitive->material()->emission->eval(ShadingPoint(uv));
		}
