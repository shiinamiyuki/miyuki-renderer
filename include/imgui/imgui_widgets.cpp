...
// The albedo used are the button colors.
...
// With typical options: Left-click on colored square to open albedo picker. Right-click to open option menu. CTRL-Click over input fields to edit them and TAB to go to next item.
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    ImRect bb_inner = bb;
    float off = -0.75f; // The border (using Col_FrameBg) tends to look off when albedo is near-opaque and rounding is enabled. This offset seemed like a good middle ground to reduce those artifacts.
    bb_inner.Expand(off);
