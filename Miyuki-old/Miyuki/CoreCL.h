...
namespace CoreCL {
	...
	public:
		...
		Context(Logger &l) { setLogger(l); create(); }
	};
	class MemObject {
	protected:
		cl_mem object;
	...
	public:
		Buffer() :MemObject() {}
		bool create(Context*,cl_mem_flags flag, size_t size);
	...
	struct Kernel {
		...
		void setWorkDimesion(unsigned int g, unsigned int l) { globalWorkSize = g; localWorkSize = l; }
		void buildProgram(const char * src, size_t size);
		void loadProgram( const char* filename);
		void createKernel( const char*file, const char* ker);
		void operator() ();
		void setArg(const Buffer&buffer,int i);
		void operator()(const Buffer&buffer) { setArg(buffer, 0); }
		void setArgV(int i, const Buffer & buffer) {
			setArg(buffer, i);
		}
		template<typename... Arg>
		void setArgV(int i, const Buffer & buffer, Arg... arg) {
			setArg(buffer, i);
			setArgV(i + 1, arg...);
		}
		template<typename... Arg>
		void operator()(const Buffer & buffer, Arg... arg) {
			setArg(buffer, 0);
			setArgV(1, arg...);
			(*this)();
		}
